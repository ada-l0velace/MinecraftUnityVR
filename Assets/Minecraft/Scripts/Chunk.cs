...
public class Chunk {
		...
		for (int z = 0; z < World.chunkSize; z++)
			for (int y = 0; y < World.chunkSize; y++)
				for (int x = 0; x < World.chunkSize; x++) {
					...
					chunkData [x, y, z].Draw (Verts, Norms, UVs, Tris, Verts_w, Norms_w, UVs_w, Tris_w);

				}

		CombineQuads2 (chunk.gameObject, Verts, Norms, UVs, Tris ,cubeMaterial);

		MeshCollider collider = chunk.gameObject.AddComponent (typeof(MeshCollider)) as MeshCollider;
		collider.sharedMesh = chunk.transform.GetComponent<MeshFilter> ().mesh;
		//if(waterChunk)
		CombineQuads2(fluid.gameObject, Verts_w, Norms_w, UVs_w, Tris_w , fluidMaterial);
	...
	public void CombineQuads2(GameObject o, List<Vector3> v, List<Vector3> n, List<Vector2> u, List<int> t, Material m) {
		Mesh mesh = new Mesh();
		mesh.name = "ScriptedMesh00"; 
